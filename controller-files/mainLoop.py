...
# set p to 0 for reading
# set p to 1 for writing
p = 0
...
while device != None:
	try:
		...
		print "EXCEPTION: TOO MANY DEVICES CREATED BY DEVICECHAIN"
	finally:
		...
		while device != None:
			...
			try:
				...
				for j in range(0, len(readDic)):
					if i == 0:
						temp[dummyColListOne[j]] = readDic[j + 1]
					if i == 1:
						temp[dummyColListTwo[j]] = readDic[j + 1]
				print str(temp)
				if i == 0:
